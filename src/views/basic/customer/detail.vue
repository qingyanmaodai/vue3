<template>
  <div>
      ...
      <Tabs v-model:activeKey="activeTabs" class="tabs">
        <TabPane key="basicInfo" tab="基本信息">
          <a-form :model="formState" :rules="formRules" ref="formRef">
            ...
            <Row>
              <Col :span="8">
                <a-form-item label="客户等级：" ref="level" name="level" class="item">
                  <Select
    ...
    <!--  客户分组弹框  -->
    <a-modal v-model:visible="supplierGroupModel" title="客户分组" ref="node">
      <a-tree-select
        loading
        show-search
        v-model:value="formState.groupId"
...
<script lang="ts" setup>
  ...
  import { getOneSupplier, save, update, audit, unAudit, SupplierEntity } from '/@/api/supplier'; //客户api
  import {
    getSupplierGroupTree,
    queryOneSupplierGroup,
    SupplierGroupEntity,
  } from '/@/api/supplierGroup'; //客户分组api
  ...
  const supplierId = ref<string>(''); //路由参数，客户Id
  ...
  const supplierGroupModel = ref<boolean>(false); //客户分组弹框显示状态
