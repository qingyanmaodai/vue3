...
<script setup lang="ts" name="warehouse-others-input-index">
  ...
  import {
    ...
    upSearch,
    pushDown,
  } from '/@/api/others/input';
  import 'splitpanes/dist/splitpanes.css';
  import { cloneDeep } from 'lodash-es';
  import { gridOptions, warOthersInputColumns } from '/@/components/ExTable/data';
  ...
  const getList = async (currPage = tablePages.currentPage, pageSize = tablePages.pageSize) => {
    ...
    const res: any = await getDataList({
      params: getParams,
      orderByBean: {
        descList: ['BsOtherIn.update_time'],
      },
      pageIndex: currPage,
      pageRows: pageSize,
    });
  ...
  const addTableEvent = () => {
    let groupId = '';
    go({
      path: PageEnum.WAR_OTHERS_INPUT_DETAIL,
      query: {
